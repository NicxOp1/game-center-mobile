...
const cartReducer = createReducer(initialState, (builder) => {
  builder
    .addCase(addGame, (state, action) => {
      ...
      let sortedProducts = newProduct.sort((p1, p2) =>
    ...
    .addCase(plus, (state, action) => {
      let arrayState = state.products.filter(
        (e) => e.name !== action.payload.name
      );
      let arrayProducts = state.products.filter(
        (e) => e.name === action.payload.name
      );
      ...
      if (arrayProducts.length > 0) {
        ...
        let sortedProducts = arrayState.sort((p1, p2) =>
          p1.name < p2.name ? 1 : p1.name > p2.name ? -1 : 0
        );
